...
const ownerEmail = `workspace-owner-${runId}@example.com`;

const memberEmail = `workspace-member-${runId}@example.com`;

const password = 'IntegrationPassword123!';

const originalWorkspaceName = `Integration Workspace ${runId}`;

const updatedWorkspaceName = `Updated Workspace ${runId}`;
...
describe('Workspace HTTP integration', () => {
  it('enforces membership, authorization, removal, and owner-only deletion', async () => {
    ...
    assert.equal(ownerRegistration.body.data.email, ownerEmail);
    ...
    assert.equal(workspaceCreation.body.data.name, originalWorkspaceName);

    const ownerWorkspaceRead = await ownerAgent
    ...
    const memberAddition = await ownerAgent
    ...
    const membershipId = memberAddition.body.data.membershipId as string;
    ...
    await memberAgent.get(`/api/workspaces/${workspaceId}`).expect(200);

    // MEMBER is below the required ADMIN role.
    await memberAgent
      .patch(`/api/workspaces/${workspaceId}`)
      .send({
        name: 'Unauthorized Workspace Name',
      })
      .expect(403);
    ...
    await memberAgent
      ...
      .send({
        confirmationName: originalWorkspaceName,
      })
      .expect(403);

    const workspaceUpdate = await ownerAgent
      .patch(`/api/workspaces/${workspaceId}`)
      .send({
    ...
    assert.equal(workspaceUpdate.body.data.name, updatedWorkspaceName);
    ...
    const removedMemberWorkspaces = await memberAgent.get('/api/workspaces').expect(200);

    assert.ok(Array.isArray(removedMemberWorkspaces.body.data));
    ...
    await memberAgent.get(`/api/workspaces/${workspaceId}`).expect(403);
    ...
    const deletion = await ownerAgent
      .delete(`/api/workspaces/${workspaceId}`)
      .send({
        confirmationName: updatedWorkspaceName,
      })
      .expect(200);

    assert.equal(deletion.body.data.id, workspaceId);

    const ownerWorkspaces = await ownerAgent.get('/api/workspaces').expect(200);
