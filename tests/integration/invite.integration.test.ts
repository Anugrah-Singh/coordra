...
const inviteeEmail = `invite-target-${runId}@example.com`;

const otherUserEmail = `invite-other-${runId}@example.com`;

const raceUserEmail = `invite-race-${runId}@example.com`;
...
const createInvite = async (data: {
  ...
  return {
    invite: response.body.data.invite as {
      id: string;
      email: string;
      status: string;
    },
...
describe('Workspace invite integration', () => {
  it('creates the workspace and test users', async () => {
    await registerAndLogin(ownerAgent, {
      email: ownerEmail,
      fullName: 'Invite Owner',
    });

    await registerAndLogin(inviteeAgent, {
      email: inviteeEmail,
      fullName: 'Invite Target',
    });

    await registerAndLogin(otherUserAgent, {
      email: otherUserEmail,
      fullName: 'Other User',
    });

    await registerAndLogin(raceUserAgent, {
      email: raceUserEmail,
      fullName: 'Race User',
    });
  ...
  it('prevents duplicate active invitations', async () => {
    ...
    assert.equal(firstInvite.invite.status, 'PENDING');
    ...
    await ownerAgent
      .delete(`/api/workspaces/${workspaceId}` + `/invites/${firstInvite.invite.id}`)
      .expect(200);
  });

  it('rejects invite acceptance by a different email address', async () => {
    const { invite, token } = await createInvite({
      email: inviteeEmail,
    });

    await otherUserAgent.post(`/api/workspace-invites/${token}/accept`).expect(403);
    ...
    await ownerAgent
      .delete(`/api/workspaces/${workspaceId}` + `/invites/${invite.id}`)
      .expect(200);
  });

  it('accepts an invitation and prevents duplicate acceptance', async () => {
    const { invite, token } = await createInvite({
      email: inviteeEmail,
      role: 'MANAGER',
    });

    const acceptance = await inviteeAgent
      .post(`/api/workspace-invites/${token}/accept`)
      .expect(200);

    assert.equal(acceptance.body.data.invite.status, 'ACCEPTED');

    assert.equal(acceptance.body.data.membership.role, 'MANAGER');

    await inviteeAgent.post(`/api/workspace-invites/${token}/accept`).expect(400);
    ...
    const [membership] = await db
      ...
      .where(
        and(
          ...
          eq(workspaceMembers.userId, createdUserIds[1]!)
        )
      )
      .limit(1);
  ...
  it('persists the EXPIRED status for an expired invite', async () => {
    ...
    await otherUserAgent.post(`/api/workspace-invites/${token}/accept`).expect(400);
  ...
  it('allows only one concurrent terminal transition', async () => {
    ...
    const [acceptResult, declineResult] = await Promise.all([
      ...
      raceUserAgent
        .post(`/api/workspace-invites/${token}/decline`)
        .then((response) => response.status),
    ]);

    const statuses = [acceptResult, declineResult].sort();
    ...
    assert.ok(storedInvite?.status === 'ACCEPTED' || storedInvite?.status === 'DECLINED');
  });

  it('allows the owner to delete the test workspace', async () => {
    assert.ok(workspaceId);
