...
import { auditApi } from '@/features/collaboration/collaboration.api';
  ...
  if (!canView) {
    return (
      <EmptyState
        ...
        description="The audit trail is restricted to workspace owners and administrators."
      />
    );
  }
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-4 border-b pb-6 sm:flex-row sm:items-end sm:justify-between [&_h1]:font-heading [&_h1]:text-3xl [&_h1]:font-semibold [&_h1]:tracking-tight [&_p]:mt-1 [&_p]:max-w-2xl [&_p]:text-muted-foreground">
        <div>
          ...
          <p>Review who changed workspace resources and when each action occurred.</p>
        </div>
        <Badge tone="success">
          <DatabaseZap size={14} /> Transactional logging
        </Badge>
      </header>
      ...
      {logsQuery.data?.length === 0 ? (
        <EmptyState
          ...
          description="Workspace changes will be written to the audit log."
        />
      ) : (
          ...
          <div className="flex flex-col">
            {logsQuery.data?.map((log) => (
              <article className="grid grid-cols-[auto_auto_1fr] gap-3" key={log.id}>
                <div className="flex w-3 flex-col items-center [&>span]:mt-3 [&>span]:size-2 [&>span]:rounded-full [&>span]:bg-primary [&>i]:mt-1 [&>i]:w-px [&>i]:flex-1 [&>i]:bg-border">
                  <span />
                  <i />
                </div>
                ...
                <div className="min-w-0 pb-7 [&>div:first-child]:flex [&>div:first-child]:flex-wrap [&>div:first-child]:gap-x-2 [&>div:first-child]:text-sm [&>div:first-child>span]:text-muted-foreground [&>p]:my-2 [&>p]:flex [&>p]:items-center [&>p]:gap-2 [&>small]:text-xs [&>small]:text-muted-foreground [&_details]:mt-3 [&_summary]:cursor-pointer [&_summary]:text-xs [&_summary]:font-medium">
                  ...
                  <p>
                    <Badge tone="neutral">{humanize(log.action)}</Badge>
                    <ArrowRight size={14} /> {humanize(log.entityType)}
                  </p>
