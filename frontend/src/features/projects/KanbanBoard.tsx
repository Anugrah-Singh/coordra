        ...
        {task.dueDate ? (
          <span
            className={cn(
              ...
              isOverdue(task.dueDate, task.status) && 'font-medium text-destructive'
            )}
          >
          ...
          <span className="inline-flex items-center gap-1 text-muted-foreground/70">
            <CalendarDays size={14} /> No due date
          </span>
        )}
        <span>
...
const KanbanColumn = ({
  ...
  return (
    <section
      ...
      className={cn(
        'min-h-[28rem] rounded-xl border bg-muted/40 p-2 transition-colors lg:min-h-[34rem]',
        droppable.isOver && 'border-primary bg-secondary/60'
      )}
      data-status={status}
    >
      ...
      <div className="flex flex-col gap-2">
        ...
        {tasks.length === 0 ? (
          <div className="grid min-h-24 place-items-center rounded-lg border border-dashed text-xs text-muted-foreground">
            Drop a task here
          </div>
        ) : null}
      </div>
