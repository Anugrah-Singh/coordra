    ...
    <div className="flex flex-col gap-6">
      ...
      {workspaces.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {workspaces.map((workspace, index) => (
            <Link
              className="group flex min-h-52 flex-col justify-between rounded-xl border bg-card p-5 text-card-foreground shadow-xs transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md"
              key={workspace.id}
          ...
          <button
            className="flex min-h-52 flex-col items-center justify-center gap-3 rounded-xl border border-dashed bg-muted/30 p-5 text-center text-muted-foreground transition-colors hover:border-primary/50 hover:bg-secondary/60 hover:text-foreground"
            type="button"
      ...
      <Card className="grid gap-6 overflow-hidden border-primary/20 bg-primary text-primary-foreground md:grid-cols-[1fr_auto] md:items-center [&_h2]:mt-2 [&_h2]:font-heading [&_h2]:text-2xl [&_p]:mt-2 [&_p]:max-w-2xl [&_p]:text-primary-foreground/70">
        <div>
          ...
          <p>
            Every project and task query is scoped to the active workspace and protected
