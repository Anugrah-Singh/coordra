...
import { inviteApi } from '@/features/collaboration/collaboration.api';
...
export const InvitePage = () => {
  ...
  const respond = async (nextAction: 'accept' | 'decline') => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : 'Unable to process invitation'
      );
    } finally {
      setAction(null);
    }
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-background p-4">
      <Card className="w-full max-w-xl gap-6 p-7 text-center shadow-lg">
        ...
        <p>
          Accept to add the workspace to your account, or decline to close the invitation.
