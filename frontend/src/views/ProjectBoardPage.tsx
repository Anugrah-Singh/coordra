...
export const ProjectBoardPage = () => {
  ...
  const pathname = usePathname() ?? `/app/workspaces/${workspaceId}/projects/${projectId}`;
    ...
    <div className="flex min-w-0 flex-col gap-5">
      <header className="flex flex-col gap-4 border-b pb-5 sm:flex-row sm:items-end sm:justify-between [&_h1]:font-heading [&_h1]:text-3xl [&_h1]:font-semibold [&_p]:mt-1 [&_p]:text-pretty [&_p]:text-muted-foreground">
        <div className="min-w-0">
          <span className="font-mono text-[10px] font-medium uppercase tracking-[.16em] text-primary">Project board</span>
      ...
      <Card size="sm" className="gap-0 py-0 shadow-xs">
        <CardHeader className="border-b py-3">
          <CardTitle className="flex items-center gap-2 text-sm"><Filter data-icon="inline-start" /><span>Board filters</span></CardTitle>
        </CardHeader>
        <CardContent className="grid gap-3 py-4 sm:grid-cols-2 lg:grid-cols-[minmax(10rem,0.8fr)_minmax(12rem,1fr)_auto_auto] lg:items-end">
          <label className="flex min-w-0 flex-col gap-1.5 text-xs font-medium text-muted-foreground">
            <span>Priority</span>
            <Select value={priority || 'ALL'} onValueChange={(value) => setPriority(value === 'ALL' ? '' : value as TaskPriority)}>
          ...
          <label className="flex min-w-0 flex-col gap-1.5 text-xs font-medium text-muted-foreground">
            <span>Assignee</span>
            <Select value={assigneeId || 'ALL'} onValueChange={(value) => setAssigneeId(value === 'ALL' ? '' : value)}>
              <SelectTrigger className="w-full bg-background"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectGroup>
                  <SelectItem value="ALL">Everyone</SelectItem>
                  {membersQuery.data?.map((member) => <SelectItem key={member.userId} value={member.userId}>{member.fullName}</SelectItem>)}
                </SelectGroup>
              </SelectContent>
            </Select>
          </label>
          <label className="flex min-h-11 cursor-pointer items-center gap-2 text-sm text-muted-foreground lg:min-h-8">
            ...
            <span className="inline-flex items-center gap-1.5"><Archive size={14} /> Include archived</span>
          </label>
      ...
      <Modal open={labelOpen} onClose={() => setLabelOpen(false)} title="Create a workspace label" description="Labels can be attached to tasks in any project in this workspace." size="sm">
        <form className="flex flex-col gap-5" onSubmit={(event) => { event.preventDefault(); createLabelMutation.mutate(); }}>
          <label className="flex flex-col gap-2 text-sm font-medium [&>small]:font-normal [&>small]:text-muted-foreground"><span>Label name</span><input className="h-9 w-full rounded-lg border border-input bg-transparent px-3 text-sm shadow-xs outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/20 disabled:cursor-not-allowed disabled:opacity-50" value={labelName} maxLength={50} onChange={(event) => setLabelName(event.target.value)} autoFocus /></label>
          <label className="flex flex-col gap-2 text-sm font-medium [&>small]:font-normal [&>small]:text-muted-foreground"><span>Label color</span><div className="flex items-center gap-3"><input className="h-9 w-full rounded-lg border border-input bg-transparent px-3 text-sm shadow-xs outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/20 disabled:cursor-not-allowed disabled:opacity-50" type="color" value={labelColor} onChange={(event) => setLabelColor(event.target.value)} /><input className="h-9 w-full rounded-lg border border-input bg-transparent px-3 text-sm shadow-xs outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/20 disabled:cursor-not-allowed disabled:opacity-50" value={labelColor} pattern="^#[0-9A-Fa-f]{6}$" onChange={(event) => setLabelColor(event.target.value)} /></div></label>
          <div className="flex flex-wrap items-center justify-end gap-2"><Button type="button" variant="ghost" onClick={() => setLabelOpen(false)}>Cancel</Button><Button type="submit" disabled={!labelName.trim()} isLoading={createLabelMutation.isPending}>Create label</Button></div>
        </form>
      </Modal>
    </div>
  );
