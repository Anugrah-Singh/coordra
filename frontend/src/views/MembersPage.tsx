  ...
  return (
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-4 border-b pb-6 sm:flex-row sm:items-end sm:justify-between [&_h1]:font-heading [&_h1]:text-3xl [&_h1]:font-semibold [&_h1]:tracking-tight [&_p]:mt-1 [&_p]:max-w-2xl [&_p]:text-muted-foreground">
        <div>
          <span className="font-mono text-[10px] font-medium uppercase tracking-[.16em] text-primary">People and access</span>
      ...
      <Card className="overflow-hidden gap-0 shadow-xs">
        <div className="min-w-[42rem] overflow-x-auto">
          <div className="grid grid-cols-4 gap-4 border-b bg-muted/50 px-5 py-3 font-mono text-[10px] uppercase tracking-wider text-muted-foreground"><span>Member</span><span>Role</span><span>Joined</span><span>Actions</span></div>
          {membersQuery.data?.map((member) => (
            <div className="grid grid-cols-4 items-center gap-4 border-b px-5 py-4 text-sm last:border-0 hover:bg-muted/40" key={member.membershipId}>
              <div className="flex min-w-0 items-center gap-3 [&>div]:min-w-0 [&_strong]:block [&_strong]:truncate [&_span]:block [&_span]:truncate [&_span]:text-xs [&_span]:text-muted-foreground"><Avatar name={member.fullName} /><div><strong>{member.fullName}</strong><span>{member.email}</span></div></div>
              <div>
              ...
              <div>
                {canAdmin && member.role !== 'OWNER' ? (
                  <button className="inline-flex size-9 shrink-0 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50" type="button" onClick={() => setRemoveTarget(member)} aria-label={`Remove ${member.fullName}`}><Trash2 size={16} /></button>
      ...
      {canAdmin ? (
        <section className="flex flex-col gap-4">
          <header className="flex items-end justify-between [&_h2]:font-heading [&_h2]:text-xl [&_h2]:font-semibold [&_p]:text-sm [&_p]:text-muted-foreground"><div><h2>Pending invitations</h2><p>Invitations expire automatically after seven days.</p></div></header>
          {(invitesQuery.data?.filter((invite) => invite.status === 'PENDING').length ?? 0) === 0 ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {invitesQuery.data?.filter((invite) => invite.status === 'PENDING').map((invite) => (
                <Card className="flex flex-wrap items-center justify-between gap-3 rounded-lg border bg-muted/30 p-4" key={invite.id}>
                  ...
                  <button className="inline-flex size-9 shrink-0 items-center justify-center rounded-lg text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50" type="button" onClick={() => deleteInviteMutation.mutate(invite.id)} aria-label="Revoke invite"><Trash2 size={16} /></button>
                </Card>
              ))}
            </div>
          )}
      ...
      <Modal open={inviteOpen} onClose={() => { setInviteOpen(false); setDemoInviteUrl(null); }} title="Invite a teammate" description="Create a secure, expiring workspace invitation." size="sm">
        {demoInviteUrl ? (
          <div className="flex flex-col gap-5">
            <div className="rounded-lg border bg-muted p-3 font-mono text-xs"><strong>Demo invitation link</strong><p>This link is only returned when DEMO_MODE is enabled.</p><code>{demoInviteUrl}</code></div>
      ...
      <Modal open={Boolean(removeTarget)} onClose={() => setRemoveTarget(null)} title="Remove member?" description={`${removeTarget?.fullName ?? 'This member'} will lose access to the workspace immediately.`} size="sm">
        <div className="flex flex-wrap items-center justify-end gap-2"><Button variant="ghost" onClick={() => setRemoveTarget(null)}>Cancel</Button><Button variant="danger" isLoading={removeMutation.isPending} onClick={() => removeTarget && removeMutation.mutate(removeTarget.membershipId)}>Remove member</Button></div>
      </Modal>
    </div>
  );
};
...
const MemberForm = ({
  ...
  <form className="flex flex-col gap-5" onSubmit={(event) => { event.preventDefault(); onSubmit(); }}>
    <label className="flex flex-col gap-2 text-sm font-medium [&>small]:font-normal [&>small]:text-muted-foreground"><span>Email address</span><input className="h-9 w-full rounded-lg border border-input bg-transparent px-3 text-sm shadow-xs outline-none transition-colors placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/20 disabled:cursor-not-allowed disabled:opacity-50" type="email" value={email} onChange={(event) => setEmail(event.target.value)} autoFocus /></label>
    <label className="flex flex-col gap-2 text-sm font-medium [&>small]:font-normal [&>small]:text-muted-foreground"><span>Workspace role</span><select className="h-9 w-full rounded-lg border border-input bg-background px-3 text-sm shadow-xs outline-none focus-visible:border-ring focus-visible:ring-2 focus-visible:ring-ring/20 disabled:cursor-not-allowed disabled:opacity-50" value={role} onChange={(event) => setRole(event.target.value as Exclude<WorkspaceRole, 'OWNER'>)}>{assignableRoles.map((item) => <option key={item} value={item}>{humanize(item)}</option>)}</select></label>
    <div className="flex flex-wrap items-center justify-end gap-2"><Button type="button" variant="ghost" onClick={onCancel}>Cancel</Button><Button type="submit" disabled={!email.trim()} isLoading={isLoading}>{submitLabel}</Button></div>
  </form>
);
